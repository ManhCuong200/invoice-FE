'use client';
import { Button } from "@/components/ui/button";


interface LoginCardProps {
    onGoogleLogin: () => void;
}

export default function LoginCard({ onGoogleLogin }: LoginCardProps) {
    return (
        <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-4">
            {/* Background Decor */}
            <div className="absolute top-1/4 left-1/4 h-72 w-72 rounded-full bg-blue-500/10 blur-3xl"></div>
            <div className="absolute bottom-1/4 right-1/4 h-72 w-72 rounded-full bg-indigo-500/10 blur-3xl"></div>

            {/* Card Container */}
            <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl shadow-2xl">

                {/* Logo & Header */}
                <div className="text-center mb-8">
                    <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600 font-black text-xl text-white shadow-lg shadow-blue-500/30">
                        X
                    </div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">
                        Hệ thống OCR Hóa Đơn
                    </h2>
                    <p className="mt-2 text-sm text-slate-400">
                        Giải pháp tự động hóa nhập liệu cho Công ty Kế toán X
                    </p>
                </div>

                {/* Info Box */}
                <div className="mb-6 rounded-lg bg-white/5 p-4 border border-white/5 text-center">
                    <span className="text-xs font-semibold text-indigo-400 tracking-wider uppercase block mb-1">
                        Năng Lực Ứng Viên
                    </span>
                    <span className="text-sm font-medium text-slate-200">
                        Trương Mạnh Cường — DTU/CMU IT Student
                    </span>
                </div>

                {/* Action Button */}
                <div className="space-y-4">
                    <Button
                        onClick={onGoogleLogin}
                        className="flex w-full items-center justify-center gap-3 rounded-xl bg-white px-4 py-3.5 text-sm font-semibold text-slate-700 shadow-md transition-all duration-200 hover:bg-slate-50 hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
                    >
                        {/* Google Vector Icon */}
                        <svg className="h-5 w-5" viewBox="0 0 24 24">
                            <path
                                fill="#4285F4"
                                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                            />
                            <path
                                fill="#34A853"
                                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                            />
                            <path
                                fill="#FBBC05"
                                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
                            />
                            <path
                                fill="#EA4335"
                                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                            />
                        </svg>
                        Tiếp tục với Google Account
                    </Button>
                </div>

                {/* Footer Note */}
                <div className="mt-8 text-center text-xs text-slate-500">
                    Hệ thống bảo mật bằng giải pháp Google SSO OAuth 2.0
                </div>
            </div>
        </div>
    );
}
